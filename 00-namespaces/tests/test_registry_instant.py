...
from namespace_registry.cache import MultiLayerCache, CacheLevel
from namespace_registry.schema_validator import SchemaValidator, SchemaValidationStatus
...
class TestRegistryValidator:
    ...
    async def test_validate_namespace_valid(self, validator, valid_namespace_data):
        ...
        result = await validator.validate_namespace(
            "platform-registry-service-v1",
            valid_namespace_data
        )
    ...
    async def test_validate_all_parallel(self, validator, valid_namespace_data):
        ...
        assert sum(r.latency_ms for r in results.values()) / len(results) < 500  # <500ms
...
class TestSchemaValidator:
    ...
    async def test_validate_all_schemas(self, validator, valid_schema):
        ...
        assert len(results) == 3
        assert all(
            r.status == SchemaValidationStatus.PASSED 
            for r in results.values()
        )
...
class TestRegistryManagerInstant:
    ...
    async def test_search_namespaces(self, registry, valid_namespace_data):
        """測試搜索 namespaces"""
        # 創建
        await registry.create_namespace("platform-registry-service", valid_namespace_data)
