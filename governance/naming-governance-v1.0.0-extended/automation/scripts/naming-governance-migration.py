...
class NamingGovernanceMigration:
    ...
    def _discover_namespace_assets(self, namespace: str) -> List[AssetInfo]:
        ...
        resource_types = [
            ...
            'statefulsets', 'daemonsets', 'replicasets', 'horizontalpodautoscalers'
        ]
    ...
    def _create_asset_info(self, item: Dict[str, Any], resource_type: str, namespace: str) -> Optional[AssetInfo]:
        """創建資產信息"""
        try:
                ...
                if resource_type in ['deployments', 'statefulsets']:
                    ...
                    if selector:
                        for potential_asset in assets:
                            if (potential_asset.resource_type in ['deployments', 'statefulsets'] and
                                potential_asset.labels.get('app') == selector):
    ...
    def _assess_deployment_complexity(self, asset: AssetInfo) -> int:
        ...
        try:
            cmd = [
                'kubectl', 'get', 'deployment', asset.name,
                '-n', asset.namespace,
                '-o', 'json'
            ]
    ...
    def create_migration_plan(self, assets: List[AssetInfo]) -> MigrationPlan:
        ...
        sorted_assets = sorted(assets, key=lambda a: (
            migration_order.index(a.resource_type) if a.resource_type in migration_order else 999,
    ...
    def execute_staged_migration(self, plan: MigrationPlan) -> bool:
        ...
        try:
            ...
            asset_batches = [plan.assets[i:i + batch_size] for i in range(0, len(plan.assets), batch_size)]
            
            successful_migrations = 0
            failed_migrations = 0
            
            for batch_index, batch in enumerate(asset_batches):
                logger.info(f"執行批次 {batch_index + 1}/{len(asset_batches)}，包含 {len(batch)} 個資產")
    ...
    def _migrate_config_resource(self, asset: AssetInfo, new_name: str) -> bool:
        ...
        try:
            # 導出當前配置
            export_cmd = [
                'kubectl', 'get', asset.resource_type, asset.name,
                '-n', asset.namespace,
                '-o', 'yaml'
            ]
            ...
            # 驗證新資源
            verify_cmd = [
                'kubectl', 'get', asset.resource_type, new_name,
                '-n', asset.namespace
            ]
            ...
            # 刪除舊資源
            delete_cmd = [
                'kubectl', 'delete', asset.resource_type, asset.name,
                '-n', asset.namespace
            ]
