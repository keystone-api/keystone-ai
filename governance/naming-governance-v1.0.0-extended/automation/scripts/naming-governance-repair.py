...
class NamingGovernanceRepair:
    ...
    def _get_all_resources(self, namespace: str) -> List[Dict[str, Any]]:
        ...
        resource_types = [
            ...
            'persistentvolumeclaims', 'ingress', 'serviceaccounts',
    ...
    def _group_violations(self, violations: List[ViolationReport]) -> Dict[ViolationType, List[ViolationReport]]:
        """按違規類型分組"""
        grouped = {}
    ...
    def _requires_approval(self, violations: List[ViolationReport]) -> bool:
        ...
        return (
            ...
            any(v.violation_type == ViolationType.SECURITY_VIOLATION for v in violations)
        )
    ...
    def _repair_naming_pattern(self, operation: RepairOperation) -> bool:
        ...
        for violation in operation.violation_reports:
            try:
                ...
                if self.dry_run:
                    logger.info(f"[DRY RUN] 將重命名 {resource_type}/{current_name} 為 {suggested_name}")
                    continue
                ...
                cmd = [
                    'kubectl', 'patch', resource_type, current_name,
                    '-n', namespace,
                    '-p', json.dumps(patch),
                    '--type', 'merge'
                ]
    ...
    def _repair_missing_labels(self, operation: RepairOperation) -> bool:
        ...
        for violation in operation.violation_reports:
            try:
                ...
                if self.dry_run:
                    logger.info(f"[DRY RUN] 將為 {resource_type}/{resource_name} 添加標籤: {missing_labels}")
                    continue
                ...
                logger.info(f"成功為 {resource_type}/{resource_name} 添加標籤: {list(labels_to_add.keys())}")
    ...
    def _repair_version_format(self, operation: RepairOperation) -> bool:
        ...
        for violation in operation.violation_reports:
            try:
                ...
                if self.dry_run:
                    logger.info(f"[DRY RUN] 將為 {resource_type}/{resource_name} 更新版本標籤為 {correct_version}")
                    continue
                ...
                logger.info(f"成功更新 {resource_type}/{resource_name} 版本標籤為 {correct_version}")
    ...
    def _verify_repair_result(self, operation: RepairOperation) -> bool:
        ...
        try:
            # 重新檢測違規
            namespace = operation.violation_reports[0].namespace if operation.violation_reports else "default"
            ...
            if not remaining_violations:
                ...
                logger.warning(f"修復操作 {operation.operation_id} 驗證失敗：仍有 {len(remaining_violations)} 個違規未解決")
                return False
    ...
    def generate_repair_report(self, operations: List[RepairOperation]) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "completed_operations": len([op for op in operations if op.status == RepairStatus.COMPLETED]),
                "failed_operations": len([op for op in operations if op.status == RepairStatus.FAILED]),
                "auto_repaired": len([op for op in operations if op.status == RepairStatus.VERIFIED])
            },
...
def main():
    ...
    parser.add_argument("--namespace", "-n", default="machine-native-ops", help="Kubernetes 命名空間")
    ...
    try:
        ...
        return 0 if all(op.status in [RepairStatus.COMPLETED, RepairStatus.VERIFIED] for op in successful_operations) else 1
