...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Analyze CI/CD workflow failures"
    )
    parser.add_argument(
        "--workflow-run-id",
        type=str,
    ...
    parser.add_argument(
        "--job-name",
        type=str,
        help="Specific job name to analyze"
    )
    parser.add_argument(
        "--log-file",
        type=str,
...
def analyze_log_content(log_content: str, source: str = "github_actions") -> Dict[str, Any]:
    """
...
def format_markdown_report(analysis: Dict[str, Any]) -> str:
    ...
    if auto_fixable:
        report.append("\n## ✅ Auto-Fixable Errors\n")
        report.append(f"Found {len(auto_fixable)} errors that can be automatically fixed:\n")
        for error in auto_fixable[:5]:  # Limit to first 5
            report.append(f"- {error.get('title', 'Unknown')} - {error.get('fix_suggestion', '')}")
...
def main():
    ...
    if not log_content:
        print("⚠️  No log content to analyze. Provide --log-file or pipe log content to stdin.")
        sys.exit(0)
