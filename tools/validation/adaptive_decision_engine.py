...
class DynamicPolicyController:
    ...
    def adjust_parameters(
        ...
        """
        logger.info(f"Adjusting parameters - Noise: {noise_level:.4f}, Coherence: {coherence_time:.4f}")
...
class FusionEngine:
    ...
    def decide(
        ...
        for dim_name, weight in dimension_weights.items():
            ...
            status = (
                ValidationStatus.PASS if hybrid_score > 0.9
                else ValidationStatus.WARNING if hybrid_score > 0.7
        ...
        # 確定整體狀態
        overall_status = (
            ValidationStatus.PASS if total_score > 0.9
            else ValidationStatus.WARNING if total_score > 0.7
            else ValidationStatus.FAIL
        )
...
def run_demo_decision() -> dict:
    ...
    return engine.execute_decision(
        classic_results,
        quantum_features,
        document_path="/demo/example-spec.json"
    )
