...
class ExceptionManager:
    ...
    def validate_exception_request(self, exception: ComplianceException) -> Tuple[bool, List[str]]:
        ...
        # 驗證必要欄位
        required_fields = [
            'applicant', 'exception_type', 'justification', 
            'risk_assessment', 'expiry_date'
        ]
    ...
    def approve_exception(self, exception_id: str, reviewer: str, notes: Optional[str] = None) -> bool:
        """批准例外申請"""
    ...
    def generate_exception_report(self, status_filter: Optional[str] = None) -> Dict:
        ...
        if status_filter:
            filtered_exceptions = [exc for exc in self.exceptions 
                                 if exc['status'] == status_filter]
        
        report = {
            'generated_at': datetime.now().isoformat(),
            'total_exceptions': len(self.exceptions),
            'by_status': {
                'pending': len([e for e in self.exceptions if e['status'] == ExceptionStatus.PENDING.value]),
                'approved': len([e for e in self.exceptions if e['status'] == ExceptionStatus.APPROVED.value]),
                'rejected': len([e for e in self.exceptions if e['status'] == ExceptionStatus.REJECTED.value]),
                'expired': len([e for e in self.exceptions if e['status'] == ExceptionStatus.EXPIRED.value]),
                'revoked': len([e for e in self.exceptions if e['status'] == ExceptionStatus.REVOKED.value])
            },
    ...
    def export_exception_form(self, exception_id: str, format: str = "yaml") -> str:
        ...
        for exc in self.exceptions:
            if exc['id'] == exception_id:
                if format == "yaml":
                    return yaml.dump(exc, allow_unicode=True, default_flow_style=False, sort_keys=False)
                elif format == "json":
...
def main():
    ...
    parser.add_argument('--spec', default='config/machine-spec.yaml', 
                       help='命名規範文件路徑')
    parser.add_argument('--db', default='exceptions-db.yaml', 
                       help='例外資料庫文件路徑')
    ...
    report_parser.add_argument('--format', choices=['yaml', 'json'], default='yaml',
                              help='輸出格式')
