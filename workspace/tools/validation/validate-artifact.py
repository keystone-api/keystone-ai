...
class ArtifactValidator:
    ...
    def __init__(self, artifact_paths: List[str], validation_level: str = "all", strict: bool = False):
    ...
    def _validate_structural(self):
        ...
        for artifact in self.artifacts:
            ...
            if not any(r.status == "fail" and r.details.get('path') == artifact_path 
    ...
    def _validate_semantic(self):
        ...
        for artifact in self.artifacts:
            ...
            artifact_failures = [r for r in self.results 
                               if r.level == "semantic" 
                               and r.status == "fail" 
    ...
    def _validate_governance(self):
        ...
        for artifact in self.artifacts:
            ...
            artifact_name = metadata.get('name', '')
            if artifact_name:
                name_pattern = naming_conventions.get('artifact_naming', {}).get('pattern', '^[a-z][a-z0-9-]*$')
                if not re.match(name_pattern, artifact_name):
                    self.results.append(ValidationResult(
                        ...
                        details={
                            ...
                            "examples": naming_conventions.get('artifact_naming', {}).get('examples', [])
                        }
                    ))
            ...
            version = metadata.get('version', '')
            if version:
                version_pattern = naming_conventions.get('version_naming', {}).get('pattern', r'^\d+\.\d+\.\d+$')
                if not re.match(version_pattern, version):
                    self.results.append(ValidationResult(
                        ...
                        details={
                            ...
                            "examples": naming_conventions.get('version_naming', {}).get('examples', [])
                        }
                    ))
            ...
            if namespace:
                namespace_pattern = naming_conventions.get('namespace_naming', {}).get('pattern', '^[a-z][a-z0-9-]*$')
    ...
    def _validate_closure(self):
        ...
        for artifact in self.artifacts:
            ...
            all_closures_passed = (
                dependency_closure_passed and 
                semantic_closure_passed and 
                governance_closure_passed and
                structural_closure_passed
            )
            
            if not all_closures_passed:
                failed_closures = [
                    level for level, status in closure_status.items() 
                    if status == "fail"
                ]
    ...
    def generate_attestation(self, output_path: Optional[str] = None) -> Dict:
        ...
        attestation = {
            ...
            "spec": {
                ...
                "validation_results": {
                    "structural": {
                        ...
                        "passed": len([r for r in results_by_level.get('structural', []) if r.status == "pass"]),
                        "failed": len([r for r in results_by_level.get('structural', []) if r.status == "fail"]),
                        "warnings": len([r for r in results_by_level.get('structural', []) if r.status == "warning"]),
                    ...
                    "semantic": {
                        ...
                        "passed": len([r for r in results_by_level.get('semantic', []) if r.status == "pass"]),
                        "failed": len([r for r in results_by_level.get('semantic', []) if r.status == "fail"]),
                        "warnings": len([r for r in results_by_level.get('semantic', []) if r.status == "warning"]),
                    ...
                    "dependency": {
                        ...
                        "passed": len([r for r in results_by_level.get('dependency', []) if r.status == "pass"]),
                        "failed": len([r for r in results_by_level.get('dependency', []) if r.status == "fail"]),
                        "warnings": len([r for r in results_by_level.get('dependency', []) if r.status == "warning"]),
                    ...
                    "governance": {
                        ...
                        "passed": len([r for r in results_by_level.get('governance', []) if r.status == "pass"]),
                        "failed": len([r for r in results_by_level.get('governance', []) if r.status == "fail"]),
                        "warnings": len([r for r in results_by_level.get('governance', []) if r.status == "warning"]),
                    ...
                    "closure": {
                        ...
                        "passed": len([r for r in results_by_level.get('closure', []) if r.status == "pass"]),
                        "failed": len([r for r in results_by_level.get('closure', []) if r.status == "fail"]),
                        "warnings": len([r for r in results_by_level.get('closure', []) if r.status == "warning"]),
                ...
                "governance_compliance": {
                    ...
                    "semantic_root_version": self.semantic_root.get('metadata', {}).get('version', 'unknown') if self.semantic_root else None,
            ...
            "status": {
                ...
                "dependency_closure": self._get_level_status(results_by_level.get('dependency', [])) == "pass",
                "semantic_closure": self._get_level_status(results_by_level.get('semantic', [])) == "pass",
                "governance_closure": self._get_level_status(results_by_level.get('governance', [])) == "pass"
            }
        }
        ...
        if output_path:
            ...
            with open(output_file, 'w') as f:
                yaml.dump(attestation, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
...
Examples:
  ...
  %(prog)s --level all artifact1.yaml artifact2.yaml
    ...
    parser.add_argument(
        'artifacts',
        nargs='*',
        help='Artifact files to validate (YAML format)'
    )
    
    parser.add_argument(
        '--attestation',
        help='Path to save attestation bundle (YAML format)'
    )
